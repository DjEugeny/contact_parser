...
### Задача 3.4: Диагностировать нормализацию телефонных номеров
**Приоритет:** 🟢 НИЗКИЙ (не критично)  
**Срок:** 1 день  
**Ответственный:** Backend Developer

**Проблема:**  
В логах появляются предупреждения о телефонах без normalized поля:
```
⚠️ Телефон без normalized поля: 8(38822)49313
⚠️ Телефон без normalized поля: 8(977)702-72-19
```

Однако в итоговых файлах телефоны записываются корректно нормализованными:
```json
"number": "+7 (388) 224-93-13"
"number": "+7 (977) 702-72-19"
```

**Гипотеза:**  
Предупреждение появляется на промежуточном этапе обработки (до нормализации), а затем телефоны корректно нормализуются. Возможно это ложное предупреждение или логирование происходит не в том месте.

**Шаги:**
1. Найти где генерируется предупреждение "Телефон без normalized поля"
2. Проверить порядок обработки телефонов: LLM → проверка → нормализация → сохранение
3. Определить является ли это проблемой или информационным сообщением
4. Если ложное предупреждение - убрать или изменить на debug уровень
5. Если реальная проблема - исправить порядок обработки
6. Протестировать на письмах email_002 и email_006

**Код для диагностики:**
```python
# Найти где генерируется предупреждение
grep -r "Телефон без normalized поля" src/

# Проверить порядок обработки
# 1. LLM возвращает телефон в формате: "8(38822)49313"
# 2. Валидатор проверяет наличие normalized поля (здесь предупреждение?)
# 3. Нормализатор преобразует: "8(38822)49313" → "+7 (388) 224-93-13"
# 4. Сохранение в processed.json

# Возможное решение 1: Убрать проверку до нормализации
# Возможное решение 2: Изменить уровень логирования на DEBUG
# Возможное решение 3: Переместить проверку после нормализации
```

**Критерии успеха:**
- [ ] Найдено место генерации предупреждения
- [ ] Определен порядок обработки телефонов
- [ ] Определена природа предупреждения (ложное или реальная проблема)
- [ ] Если ложное - убрано или изменено на DEBUG
- [ ] Если реальная проблема - исправлен порядок обработки
- [ ] Логи стали чище (нет ложных предупреждений)
- [ ] Нормализация продолжает работать корректно

**Файлы для тестирования:**
- `llm_results/2025-10-03/email_002_20251003_20251003_mail_ru_f17b42db_20251008_223645_223720_processed.json`
- `llm_results/2025-10-03/email_006_20251003_20251003_dna_technology_ru_3a2d666b_20251008_223645_223833_processed.json`
- `api_pipeline_validator_20251008_223616_unknown.log`

---

## Фаза 4: Долгосрочные улучшения (2-4 недели)
...
- [ ] Dashboard показывает текущее состояние
...
**Шаги:**
1. Включить кэширование после отладки
2. Исследовать параллельную обработку
3. Оптимизировать OCR
4. Исследовать batch processing для LLM
5. Замерить улучшения
...
2. Убедиться что переменные окружения читаются
3. Добавить логирование при инициализации
4. Протестировать обогащение

**Критерии успеха:**
- [ ] DaData клиент инициализируется корректно
- [ ] Обогащение данных работает
...
2. Создать config/paths.py
3. Обновить затронутые модули
4. Физически переименовать папку
5. Протестировать
...
**Средних:** 4 задачи (Фаза 3)  
**Низких:** 3 задачи (Фаза 4)  
...
**Дата последнего обновления:** 2025-10-08  
**Статус:** Обновлен после исследования и добавления задачи по нормализации телефонов. Определить является ли это проблемой или просто информационным сообщением
4. Если проблема - исправить порядок обработки
5. Если не проблема - убрать/изменить предупреждение на debug уровень
6. Добавить unit-тесты для нормализации телефонов
...
# Проверить порядок обработки
# 1. Телефон приходит от LLM
# 2. Проверка наличия normalized поля (здесь предупреждение?)
# 3. Нормализация телефона
# 4. Сохранение в результат
```

**Критерии успеха:**
- [ ] Найдено место генерации предупреждения
- [ ] Определена природа проблемы
- [ ] Если ложное предупреждение - убрано или изменено на debug
- [ ] Если реальная проблема - исправлен порядок обработки
- [ ] Добавлены тесты для нормализации
