...
# -*- coding: utf-8 -*-
"""
🔍 Упрощенный скрипт для анализа писем и формирования конфигурации PreCleaner.
...
class SimpleEmailPatternAnalyzer:
    """🔍 Упрощенный анализатор паттернов в письмах"""
    ...
    def analyze_emails(self, limit_days: int = 7) -> Dict:
        ...
        date_dirs = sorted([d for d in self.emails_dir.iterdir() if d.is_dir()])[-limit_days:]
    ...
    def generate_precleaner_config(self, output_path: str = "config/precleaner.yaml") -> Dict:
        ...
        # Формируем базовую конфигурацию с найденными паттернами
        base_sig_markers = ['--', '—', 'с уважением', 'best regards', 'kind regards', 'regards']
        base_disclaimer_markers = [
        ...
        # Добавляем найденные маркеры к базовым
        sig_markers.update(base_sig_markers)
        disclaimer_markers.update(base_disclaimer_markers)
        ...
        config = {
            ...
            '_metadata': {
                ...
                'found_signature_patterns': len(self.patterns['signatures']),
                'found_disclaimer_patterns': len(self.patterns['disclaimers']),
                'found_quote_patterns': len(self.patterns['quotes'])
            }
        }
...
def main():
    ...
    # Создаем анализатор
    analyzer = SimpleEmailPatternAnalyzer()
