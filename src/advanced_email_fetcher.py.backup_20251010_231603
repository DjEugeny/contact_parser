...
class AdvancedEmailFetcherV2:
        ...
        try:
            ...
            if 0 <= size < 1_000_000_000:
                ...
                if size > 1_000_000_000:
                    self.logger.warning(
                        f"   ⚠️ Подозрительно большой размер: {size} байт"
                    )
                return 0
        ...
        try:
            ...
            if status == "OK" and data:
                ...
                if m:
                    ...
                    if self.enable_size_logging:
                        self.logger.info(
                            f"🔍 Общий размер письма: {real_size} байт ({real_size/1024:.1f} КБ)"
                        )
                    ...
                    if self.enable_size_logging:
                        self.logger.warning(
                            f"⚠️ Не удалось распарсить размер письма из строки: {size_str}"
                        )
                    return -1
    ...
    def save_skipped_email(self, msg_id: bytes, date_str: str, reason: str = "unknown"):
        ...
        if attempts >= 3:
            self.logger.warning(
                f"⚠️ Письмо {key} уже имеет {attempts} попыток, не добавляем в очередь"
            )
            return
        ...
        self.logger.info(
            f"💾 Письмо {key} сохранено для повторной обработки (попытка {attempts + 1}/3, причина: {reason})"
        )
    ...
    def retry_skipped_emails(self):
        ...
        self.logger.info("=" * 70)
        self.logger.info(
            f"🔄 ПОВТОРНАЯ ОБРАБОТКА ПРОПУЩЕННЫХ ПИСЕМ: {len(skipped)} в очереди"
        )
        self.logger.info("=" * 70)
        ...
        for key in list(skipped.keys()):
            ...
            if attempts >= 3:
                self.logger.warning(
                    f"🆘 Письмо {key} достигло лимита попыток ({attempts}), перенос в мертвую очередь"
                )
                msg_id = key.encode() if isinstance(key, str) else key
                self.move_to_dead_letter(
                    msg_id, f"исчерпаны попытки после {attempts} раз - {reason}"
                )
            ...
            try:
                ...
                email_data = self.process_single_email(
                    msg_id, date_str, 1, 1, include_attachment_data=False
                )
            ...
            try:
                with open(dead_letter_path, "r", encoding="utf-8") as f:
                    ...
                    for key, info in dead_letters.items():
                        ...
                        self.logger.info(
                            f"  {key}: {info['reason']} (перенесено: {moved_at})"
                        )
                    return dead_letters
            except Exception as e:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    self.logger.warning(
                        f"   ⚠️ Все попытки исчерпаны, пробуем fallback..."
                    )
                ...
                if attempt < MAX_RETRIES - 1:
                    self.logger.info(
                        f"   🔄 Переподключение через {RETRY_DELAY} сек..."
                    )
                    time.sleep(RETRY_DELAY)
                    if not self.connect():
                        continue
    ...
    def try_fallback_fetch(self, msg_id: bytes) -> Optional[List]:
        ...
        try:
            ...
            if status == "OK" and data:
                self.logger.info(
                    "   ✅ Fallback частично успешен - загружены только заголовки"
                )
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if raw_headers is None:
                    self.logger.error(
                        f"   ❌ Не удалось извлечь сырые заголовки из ответа IMAP"
                    )
                    return None
                ...
                self.logger.warning(
                    f"   ⚠️ Сетевая ошибка загрузки заголовков (попытка {attempt + 1}): {e}"
                )
                if attempt < MAX_RETRIES - 1:
        ...
        try:
            ...
            if isinstance(header_data, (list, tuple)):
                ...
                try:
                    if (
                        len(header_data) > 0
                        and isinstance(header_data[0], bytes)
                        and len(header_data) > 50
                    ):
                        return header_data
                except (IndexError, TypeError):
                    pass
    ...
    def analyze_bodystructure(self, msg_id: bytes) -> Dict:
        ...
        try:
            ...
            return {"has_large_attachments": has_large, "structure": structure_str}
    ...
    def detect_large_attachments_from_structure(self, structure_str: str) -> bool:
        ...
        try:
            ...
            if self.enable_size_logging:
                ...
                self.logger.info(
                    f"📋 Начальные символы структуры: {structure_str[:500]}..."
                )

            if (
                "attachment" not in structure_lower
                and "application" not in structure_lower
            ):
                if self.enable_size_logging:
            ...
            for indicator in large_indicators:
                if indicator in structure_lower:
                    if self.enable_size_logging:
                        self.logger.info(
                            f"⚠️ Обнаружен потенциально большой файл: {indicator}"
                        )
                    return True
            ...
            for size_str in size_matches:
                ...
                if (
                    len(size_str) == 8
                    and "boundary" in structure_lower
                    and size_str in structure_lower
                ):
                    if self.enable_size_logging:
            ...
            if self.enable_size_logging:
                ...
                if suspicious_numbers:
                    self.logger.info(
                        f"🔍 Подозрительные большие числа (игнорируем): {suspicious_numbers[:5]}..."
                    )
            ...
            for size in valid_sizes:
                if size > 20_000_000:  # 20MB
                    if self.enable_size_logging:
                        self.logger.info(
                            f"⚠️ Обнаружен большой компонент: {size} байт ({size/1024/1024:.1f} МБ)"
                        )
                    return True
    ...
    def save_attachment_or_inline(
        ...
        try:
            ...
            if not filename and is_inline and content_type.startswith("image/"):
                ...
                self.logger.info(
                    f"🖼️ Встроенное изображение без имени, создаем: {filename}"
                )
            ...
            if is_inline:
                ...
                inline_exclusion = self.filters.is_inline_image_excluded(
                    filename, content_type, content_id
                )
                if inline_exclusion:
                    self.logger.info(
                        f"🚫 INLINE ИЗОБРАЖЕНИЕ ИСКЛЮЧЕНО: {filename} - {inline_exclusion}"
                    )
            ...
            if filename in self.specific_excluded_files:
                self.logger.info(
                    f"🚫 ИСКЛЮЧЕНО ПО ИМЕНИ ФАЙЛА: {filename} - в списке специальных исключений"
                )
                self.stats["excluded_filenames"] += 1
                return {
                    "original_filename": filename,
            ...
            if base_filename in self.specific_excluded_files:
                self.logger.info(
                    f"🚫 ИСКЛЮЧЕНО ПО БАЗОВОМУ ИМЕНИ ФАЙЛА: {filename} (база: {base_filename}) - в списке специальных исключений"
                )
                self.stats["excluded_filenames"] += 1
                return {
                    "original_filename": filename,
            ...
            if filename_exclusion:
                self.logger.info(
                    f"🚫 ИСКЛЮЧЕНО ПО ФИЛЬТРУ ИМЕНИ: {filename} - {filename_exclusion}"
                )
                self.stats["excluded_filenames"] += 1
                return {
                    "original_filename": filename,
            ...
            if is_inline or (
                ...
                in [".png", ".jpg", ".jpeg", ".gif", ".bmp", ".tiff", ".webp"]
            ):
                try:
                    ...
                    if payload:
                        ...
                        for pattern, threshold in size_thresholds.items():
                            if (
                                filename
                                and filename.lower().startswith(pattern)
                                and file_size < threshold
                            ):
                                self.logger.info(
                                    f"🚫 ИСКЛЮЧЕН МЕЛКИЙ МУСОРНЫЙ ФАЙЛ: {filename} - {file_size} байт (паттерн: {pattern})"
                                )
                        ...
                        if file_size > 10_000_000:  # 10MB для отдельного файла
                            self.logger.info(
                                f"🚫 ИСКЛЮЧЕНО: файл слишком большой - {file_size} байт"
                            )
                            self.stats["excluded_by_size"] += 1
                            return {
                                "original_filename": filename,
                        ...
                        if file_size == 83509:
                            self.logger.info(
                                f"🚫 ИСКЛЮЧЕНО ПО РАЗМЕРУ ФАЙЛА: {filename} - {file_size} байт (мусорное изображение)"
                            )
                            self.stats["excluded_by_size"] += 1
                            return {
                                "original_filename": filename,
                        ...
                        try:
                            ...
                            if width == 416 and height == 250:
                                self.logger.info(
                                    f"🚫 ИСКЛЮЧЕНО ПО РАЗМЕРАМ ИЗОБРАЖЕНИЯ: {filename} - {width}×{height} пикселей"
                                )
                            ...
                            self.logger.warning(
                                f"⚠️ Не удалось проанализировать изображение {filename}: {e}"
                            )
                            pass

                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Ошибка анализа размера файла {filename}: {e}"
                    )
            ...
            if file_ext in EXCLUDED_EXTENSIONS:
                self.logger.info(
                    f"🚫 ИСКЛЮЧЕНО ПО РАСШИРЕНИЮ: {filename} - расширение {file_ext} в черном списке"
                )
            ...
            if file_ext not in SUPPORTED_ATTACHMENTS:
                self.logger.info(
                    f"⚠️ НЕПОДДЕРЖИВАЕМОЕ РАСШИРЕНИЕ: {filename} - {file_ext}"
                )
            ...
            if existing_files:
                self.logger.info(
                    f"📁 ФАЙЛ УЖЕ СУЩЕСТВУЕТ: {filename} - найдено {len(existing_files)} файл(ов)"
                )
                # Возвращаем информацию о существующем файле
                existing_file = existing_files[0]
                file_size = (
                    existing_file.stat().st_size if existing_file.exists() else 0
                )
                ...
                return {
                    ...
                    "file_size": file_size,
                    "file_type": SUPPORTED_ATTACHMENTS.get(
            ...
            self.logger.error(
                f"❌ Ошибка сохранения {'встроенного изображения' if is_inline else 'вложения'} {filename}: {e}"
            )
            return None
    ...
    def process_single_email(
        ...
        try:
            ...
            if not headers_msg:
                ...
                self.stats["errors"] += 1
                self.save_skipped_email(
                    msg_id, date_str, "failed_to_load_headers"
                )  # ✅ ДОБАВИТЬ
                return None
            ...
            try:
                ...
                to_addr = ", ".join(to_emails[:3]) + (
                    "..." if len(to_emails) > 3 else ""
                )
                cc_addr = ", ".join(cc_emails[:3]) + (
                    "..." if len(cc_emails) > 3 else ""
                )
            ...
            if not message_id:
                ...
                self.logger.warning(
                    f"⚠️ Message-ID отсутствует, генерируем: {message_id}"
                )
            ...
            if processing_scenario == "skip_all":
                ...
                self.logger.info(
                    f"📁 ✅ ПИСЬМО УЖЕ ПОЛНОСТЬЮ ОБРАБОТАНО (JSON + вложения существуют)"
                )
                ...
                self.logger.warning(
                    f"⚠️ ОБНАРУЖЕН СЦЕНАРИЙ download_json - ЭТО ОШИБКА ЛОГИКИ"
                )
                self.logger.warning(f"   Message-ID: {message_id}")
                self.logger.warning(
                    f"   Согласно новой логике, вложений быть не должно"
                )
            ...
            self.logger.info(
                f"🔍 ОБРАБОТКА ПИСЬМА {email_num_in_day}/{total_emails_in_day}, {email_date_formatted}"
            )
            ...
            if not to_emails and to_addr:
                self.logger.warning(
                    f"⚠️ ДИАГНОСТИКА: to_emails пуст, но to_addr = {to_addr}"
                )
            if not cc_emails and cc_addr:
                self.logger.warning(
                    f"⚠️ ДИАГНОСТИКА: cc_emails пуст, но cc_addr = {cc_addr}"
                )

            # 🚫 ФИЛЬТР 3: Массовая рассылка
            mass_mailing_filter = self.filters.is_internal_mass_mailing(
                from_addr, to_emails, cc_emails
            )
            ...
            if email_size == -1:
                if self.enable_size_logging:
                    self.logger.warning(
                        "⚠️ Размер письма неизвестен, полагаемся на анализ структуры"
                    )
                ...
                self.save_skipped_email(
                    msg_id, date_str, f"large_email_{email_size}_bytes"
                )
                return None
                ...
                if self.enable_size_logging:
                    self.logger.info(
                        f"📏 Большое письмо ({email_size} байт), обрабатываем осторожно"
                    )
            else:
                if self.enable_size_logging:
            ...
            if email_size != -1 and email_size < 50_000_000:  # 50MB
                ...
                if self.enable_size_logging:
                    self.logger.info(
                        f"📊 Нормальный размер письма ({email_size} байт), загружаем все вложения"
                    )
            else:
            ...
            if scenario == "skip_all":
                self.logger.info(
                    f"⏭️ ПИСЬМО УЖЕ ОБРАБОТАНО - пропускаем (JSON и вложения существуют)"
                )
                self.stats["skipped_already_processed"] = (
                    self.stats.get("skipped_already_processed", 0) + 1
                )
                return None
            ...
            if has_large_attachments:
                self.logger.warning(
                    f"⚠️ Обнаружены большие вложения, загружаем без них..."
                )
                msg = headers_msg
                try:
                    ...
                    if status == "OK" and text_data:
                        ...
                        if raw_text:
                            body_text = raw_text.decode("utf-8", errors="ignore")[
                                :10000
                            ]
                        else:
                        ...
                        body_text = (
                            f"[Письмо с большими вложениями - текст из заголовков]"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"   ⚠️ Ошибка загрузки текста большого письма: {e}"
                    )
                ...
                if not fetch_data:
                    ...
                    self.stats["errors"] += 1
                    self.save_skipped_email(
                        msg_id, date_str, "failed_to_fetch"
                    )  # ✅ ДОБАВИТЬ
                    return None
                ...
                if not raw_email:
                    self.logger.error(
                        f"❌ Не удалось извлечь сырой байтовый поток письма"
                    )
                    self.stats["errors"] += 1
                    self.save_skipped_email(
                        msg_id, date_str, "failed_to_extract_raw"
                    )  # ✅ ДОБАВИТЬ
                    return None
                ...
                try:
                    ...
                    self.stats["errors"] += 1
                    self.save_skipped_email(
                        msg_id, date_str, f"parsing_error_{type(e).__name__}"
                    )  # ✅ ДОБАВИТЬ
                    return None
                ...
                if scenario in ["download_attachments", "download_all"]:
                                ...
                                try:
                                    ...
                                    if content_disposition == "attachment":
                                        ...
                                        if content_id or (
                                            ...
                                            self.logger.debug(
                                                f"⏭️ Пропускаем подозрительное изображение: {filename} (нет Content-ID, короткое имя)"
                                            )
                                            continue
                                    ...
                                    if is_attachment:
                                        ...
                                        attachment_info = (
                                            self.save_attachment_or_inline(
                                        ...
                                        if attachment_info:
                                            ...
                                            if status == "saved":
                                                ...
                                                status == "excluded_by_image_dimensions"
                                            ):
                                                attachments_stats[
                                                    "excluded_by_image_dimensions"
                                                ] += 1
                                    ...
                                    self.logger.warning(
                                        f"⚠️ Ошибка обработки части {part_num}: {e}"
                                    )
                                    continue
                    ...
                    self.logger.warning(
                        "⚠️ ВНИМАНИЕ: Сценарий download_json - возможная ошибка в логике определения статуса"
                    )
                    self.logger.warning(f"   Thread-ID: {thread_id}")
                    self.logger.warning(
                        "   Возможно, старая версия кода оставила файлы вложений"
                    )
                    ...
                    try:
                        ...
                        if attachments_path.exists():
                            ...
                            if attachment_files:
                                self.logger.warning(
                                    f"⚠️ Найдены существующие вложения для {thread_id}: {len(attachment_files)} файлов"
                                )
                                ...
                                for attachment_file in attachment_files:
                                    ...
                                    attachment_info = {
                                        ...
                                        "size": file_size,
                                        "path": str(
                                            attachment_file.relative_to(self.data_dir)
                                        ),
                                        "status": "saved",
                                ...
                                self.logger.warning(
                                    f"📎 Восстановлено {len(attachment_files)} существующих вложений"
                                )
                            else:
                                self.logger.info(
                                    f"📎 Для thread_id {thread_id} вложений не найдено"
                                )
                        else:
                            self.logger.info(
                                f"📎 Папка вложений не найдена: {attachments_path}"
                            )

                    except Exception as e:
                        self.logger.error(
                            f"❌ Ошибка сбора информации о существующих вложениях: {e}"
                        )
                else:
                    self.logger.info(
                        f"⏭️ Пропускаем обработку вложений (сценарий: {scenario})"
                    )
            ...
            email_data = {
                ...
                "date": date,
                "parsed_date": (
                    email_date.isoformat() if "email_date" in locals() else None
                ),
                "body": body_text,
            ...
            if scenario in ["download_json", "download_all"]:
                ...
                try:
                    ...
                    self.stats["errors"] += 1
                    self.save_skipped_email(
                        msg_id, date_str, f"save_error_{type(e).__name__}"
                    )
                    return None
                ...
                try:
                    ...
                    if existing_json_path:
                        ...
                        existing_data["attachments_stats"] = email_data[
                            "attachments_stats"
                        ]
                        ...
                        self.logger.info(
                            f"✅ Обновлен JSON с вложениями: {existing_json_path.name}"
                        )
                    else:
                        self.logger.warning(
            ...
            self.logger.error(
                f"❌ КРИТИЧЕСКАЯ ОШИБКА обработки письма {email_num_in_day}: {e}"
            )
            ...
            self.save_skipped_email(
                msg_id, date_str, f"critical_error_{type(e).__name__}"
            )
    ...
    def fetch_emails_by_date_range(
        ...
        self.logger.info(
            f"   🚫 Фильтры активны: {len(self.filters.subject_filters)} тем, {len(self.filters.blacklist)} адресов"
        )
        self.logger.info(
            f"   ✅ Поддерживаемых расширений: {len(SUPPORTED_ATTACHMENTS)}"
        )
        self.logger.info(f"   🚫 Исключенных расширений: {len(EXCLUDED_EXTENSIONS)}")
        self.logger.info(
            f"   📝 Исключений по именам: {len(self.filters.filename_excludes)}"
        )
        ...
        while current_date <= end_date:
            ...
            if len(msg_ids) > 0:
                ...
                for day_email_num, msg_id in enumerate(msg_ids, 1):
                    if (
                        processed_emails_total > 0
                        and processed_emails_total % BATCH_SIZE == 0
                    ):
    ...
    def print_final_stats(self):
        ...
        self.logger.info(
            f"📁 Уже существовало писем: {self.stats.get('already_exists', 0)}"
        )
        self.logger.info(
            f"⏭️ Пропущено уже обработанных: {self.stats.get('skipped_already_processed', 0)}"
        )
        self.logger.info(f"🚫 Исключено по теме: {self.stats['filtered_subject']}")
        self.logger.info(
            f"🚫 Исключено по черному списку: {self.stats['filtered_blacklist']}"
        )
        self.logger.info(
            f"🚫 Исключено массовых рассылок: {self.stats['filtered_mass_mailing']}"
        )
        self.logger.info("")
        self.logger.info("📎 СТАТИСТИКА ВЛОЖЕНИЙ:")
        self.logger.info(f"✅ Скачано вложений: {self.stats['saved_attachments']}")
        self.logger.info(
            f"🖼️ Встроенных изображений: {self.stats['saved_inline_images']}"
        )
        self.logger.info(
            f"🚫 Исключено по расширению: {self.stats['excluded_attachments']}"
        )
        self.logger.info(
            f"📝 Исключено по имени файла: {self.stats['excluded_filenames']}"
        )
        self.logger.info(
            f"📏 Исключено по размеру файла: {self.stats['excluded_by_size']}"
        )
        self.logger.info(
            f"🖼️ Исключено по размерам изображения: {self.stats['excluded_by_image_dimensions']}"
        )
        self.logger.info(f"⚠️ Неподдерживаемых: {self.stats['unsupported_attachments']}")

        total_attachments = (
            self.stats["saved_attachments"]
        ...
        if total_attachments > 0:
            saved_total = (
                self.stats["saved_attachments"] + self.stats["saved_inline_images"]
            )
        ...
        self.logger.info(
            f"📏 Пропущено больших писем: {self.stats['skipped_large_emails']}"
        )
                ...
                try:
                    with open(json_file, "r", encoding="utf-8") as f:
                        ...
                        if stored_message_id and stored_message_id == message_id:
                            self.logger.info(
                                f"📁 Дубликат найден по Message-ID в файле: {json_file.name}"
                            )
                            return True

                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Ошибка проверки файла {json_file.name}: {e}"
                    )
                    continue
                    ...
                    try:
                        with open(json_file, "r", encoding="utf-8") as f:
                            ...
                            if stored_message_id and stored_message_id == message_id:
                                ...
                                attachments_path = (
                                    self.data_dir / "attachments" / date_folder
                                )
                                if attachments_path.exists():
    ...
    def check_attachments_exist_for_date(self, date_folder: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Ошибка проверки вложений для даты {date_folder}: {e}"
            )
            return False
    ...
    def get_processing_scenario(self, message_id: str, date_folder: str) -> str:
        ...
        if status["json_exists"] and status["attachments_exist"]:
            ...
            self.logger.warning(
                f"⚠️ get_processing_scenario: attachments_exist=True для нового письма {message_id}"
            )
...
def test_inline_exclusion():
    ...
    test_cases = [
        ...
        (
            ...
            "image/jpeg",
            "",
            "соответствует паттерну исключения",
        ),
        ...
        (
            ...
            "",
            None,
        ),  # Это должно быть исключено по другому механизму
    ...
    for filename, content_type, content_id, expected in test_cases:
        ...
        print(
            f"{filename:<25} | {content_type:<15} | {content_id:<10} | {(result or ''):<40} | {status}"
        )
...
def cli_menu() -> tuple[datetime, datetime]:
    ...
    Returns:
        Tuple[datetime, datetime]: (start_date, end_date) или (None, None) при выходе
    """
    print("=" * 70)
...
def main():
    ...
    parser.add_argument(
        "--start-date", type=str, help="Начальная дата диапазона (различные форматы)"
    )
    parser.add_argument(
        "--end-date", type=str, help="Конечная дата диапазона (различные форматы)"
    )

    args = parser.parse_args()

    # Определяем период для обработки
    # Если нет аргументов - запускаем интерактивный режим по умолчанию
    if args.interactive or (
        not args.date and not args.start_date and not args.end_date
    ):
        # Интерактивный режим
    ...
    logger.info("=" * 75)
    logger.info(
        f"🎯 Тестовый период: {start_date.strftime('%d.%m.%Y')} - {end_date.strftime('%d.%m.%Y')}"
    )
    ...
    logger.info(
        f"🔧 ИСПРАВЛЕНЫ: черный список, decode_header_value, extract_raw_email, фильтрация GIF, специальные файлы"
    )

    # Создаем парсер с логгером
    fetcher = AdvancedEmailFetcherV2(logger=logger)
    fetcher.enable_size_logging = (
        False  # ✅ Включить детальные логи - True, ✅ Отключить детальные логи - False
    )
    ...
    try:
        ...
        if emails:
            ...
            logger.info(
                "   ✅ ИСПРАВЛЕН decode_header_value для правильного извлечения email"
            )
            logger.info(
                "   ✅ ИСПРАВЛЕН extract_raw_email для корректной обработки bytes"
            )
            logger.info("   ✅ Убран дублированный код в process_single_email")
            logger.info(
                "   ✅ ИСПРАВЛЕНА фильтрация GIF файлов через EXCLUDED_EXTENSIONS"
            )
            logger.info(
                "   ✅ ИСПРАВЛЕНО исключение проблемных файлов через specific_excluded_files"
            )
