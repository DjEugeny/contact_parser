...
- **Привязка к данным**: список дат формируется только по фактическим папкам `data/attachments`
...
### 1. Выбор даты
- Сканирование папки `data/attachments/`
- Сортировка дат по убыванию
- Отображение интерактивного меню

### 2. Обработка писем
- Чтение JSON-файлов писем
- Извлечение тела письма с fallback-последовательностью `body_clean → body_plain → body_text → body_raw`
- Подсчёт символов и токенов для тела письма

### 3. Обработка вложений
- В отчёт попадают только вложения со статусами `saved/already_exists` и реально существующие в `data/attachments/<дата>`
- Поиск OCR-результатов в `data/ocr/texts/`
- Подсчёт символов и токенов для обработанных вложений
- Отметка вложений без OCR (отображаются как `(нет OCR)`)

### 4. Кэширование
- Вычисление MD5-хэша каждого файла
- Сравнение с сохранёнными хэшами
- Пропуск неизменённых файлов с учётом версии кэша (автоинвалидация при обновлении логики подсчёта)
