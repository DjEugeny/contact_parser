...
def check_environment_variables():
    ...
    try:
        ...
        project_root = Path(__file__).parent.parent.parent  # Поднимаемся на уровень выше из src/providers/
...
def check_dotenv_file():
    ...
    project_root = Path(__file__).parent.parent.parent  # Корень проекта (поднимаемся из src/providers/)
    env_files = [
        project_root / ".env",
...
def test_openrouter_direct():
    ...
    print(f"   ⚠️ ВАЖНО: Проверьте настройки приватности на https://openrouter.ai/settings/privacy")
    print(f"   📋 Для бесплатных моделей нужно разрешить 'Free model publication'")
    ...
    try:
        ...
        for model in models_to_test:
            ...
            if response.status_code == 200:
                ...
                if "data policy" in error_text.lower():
                    print(f"      ❌ Модель {model}: Проблема с настройками приватности!")
                    print(f"      💡 Решение: https://openrouter.ai/settings/privacy")
                    print(f"      📋 Включите 'Allow free models to be published'")
                else:
                    print(f"      ⚠️ Модель {model}: {error_text[:100]}")
            elif response.status_code == 429:
                print(f"      🚫 Модель {model}: Лимит исчерпан")
            else:
                print(f"      ⚠️ Модель {model}: Статус {response.status_code}")
        
        return  # Все модели протестированы
