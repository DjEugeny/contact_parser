...
- Ищем OCR-тексты в `data/ocr/texts/YYYY-MM-DD/`
...
## 
11. Система обогащения телефонов контактов (ContactPhoneEnricher)

**Файл:** `contact_phone_enricher.py`  
**Назначение:** Безопасное обогащение контактов телефонами от связанных организаций с системой scoring

### Описание модуля

ContactPhoneEnricher реализует интеллектуальную систему обогащения контактов телефонными номерами от связанных организаций. Модуль использует многофакторную систему scoring для принятия решений об обогащении и обеспечивает безопасность данных через фильтрацию типов телефонов и проверку дубликатов.

### Архитектура системы

```
📞 Contact Phone Enrichment System
├── 🎯 Основной обогатитель (contact_phone_enricher.py)
├── 📊 Система scoring (confidence calculation)
├── 🔍 Фильтрация телефонов (phone type filtering)
├── 🛡️ Проверка дубликатов (duplicate detection)
└── 📈 Метаданные и статистика
```

### Алгоритм работы

Система обогащения работает по следующему алгоритму:

1. **Итерация по контактам** - обработка каждого контакта индивидуально
2. **Проверка связи с организацией** - наличие organization_id
3. **Получение организации** - поиск в индексе организаций
4. **Проверка телефонов организации** - наличие непустого массива phones
5. **Вычисление confidence score** - многофакторная оценка уверенности
6. **Принятие решения** - на основе порогов confidence
7. **Фильтрация телефонов** - по разрешенным типам
8. **Проверка дубликатов** - сравнение с существующими телефонами
9. **Добавление телефонов** - с метаданными обогащения

### Система scoring

Система использует многофакторную модель для вычисления уровня уверенности (confidence) в диапазоне 0.0-1.0:

#### Факторы scoring:

- **Корпоративный email** (+0.3): Email контакта содержит домен организации
- **Наличие должности** (+0.2): Поле position заполнено и не пустое
- **Роль в сообщении** (+0.2): role_in_message = "sender" или "recipient"
- **Совпадение города** (+0.15): Город контакта совпадает с городом организации
- **Высокий value_score** (+0.15): value_score контакта ≥ 7

#### Логика принятия решений:

- **confidence < 0.5** (min_confidence_threshold) → **reject** (отклонить)
- **0.5 ≤ confidence < 0.75** (review_threshold) → **needs_review** (требует проверки)
- **confidence ≥ 0.75** → **auto_accept** (автоматическое принятие)

#### Режимы обогащения:

- **conservative**: Повышенные пороги (min: 0.75, review: 0.85)
- **balanced**: Стандартные пороги (min: 0.5, review: 0.75)
- **aggressive**: Пониженные пороги (min: 0.4, review: 0.6)

### Фильтрация телефонов

Система фильтрует телефоны организации для безопасного обогащения:

#### Разрешенные типы (по умолчанию):
- **main**: Основные номера организации
- **office**: Офисные номера
- **fax**: Факсы

#### Исключаемые типы:
- **mobile**: Мобильные номера (риск персональных данных)

#### Обработка телефонов без типа:
- Телефоны с `type = null` обрабатываются как "main"
- Применяется штраф к confidence (-0.1)
- Сохраняется оригинальный тип в метаданных

### Проверка дубликатов

Система предотвращает дублирование телефонов:

#### Логика проверки:
- Сравнение по полю `normalized` (E.164 формат)
- **Приоритет LLM-источника**: Телефоны с source="llm" имеют приоритет
- Логирование всех обнаруженных дубликатов

#### Результат проверки:
- **is_duplicate**: true/false
- **reason**: Причина (llm_source_priority, duplicate_from_X)
- **existing_phone**: Существующий телефон (если найден)
...
CONTACT_PHONE_ENRICHMENT_CONFIG = {
    ...
    'min_confidence_threshold': 0.5,              # Минимальный порог confidence
    'review_threshold': 0.75,                     # Порог для автоматического принятия
    'allowed_phone_types': ['main', 'office', 'fax'],  # Разрешенные типы телефонов
    'enrichment_mode': 'balanced'                 # Режим: conservative/balanced/aggressive
}
```
...
```python
from postprocessing.contact_phone_enricher import ContactPhoneEnricher

# Создание обогатителя с конфигурацией
enricher = ContactPhoneEnricher(config=CONTACT_PHONE_ENRICHMENT_CONFIG)
...
contacts = [
    {
        ...
        "position": "Менеджер",
        "emails": [{"email": "ivanov@company.ru"}],
        "phones": [],
        "city": "Москва",
        "value_score": 8
    }
...
# Результат: контакт получит телефон с высоким confidence (0.8)
# Факторы: корпоративный email (+0.3) + должность (+0.2) + город (+0.15) + высокий value_score (+0.15)
```

#### Обработка edge cases:

```python
# Контакт без organization_id
contact_no_org = {
    "gid": "contact_002",
    "name": "Петров Петр",
    "organization_id": None,  # Нет связи с организацией
    "phones": []
}
# Результат: пропуск с причиной "no_organization"

# Организация без телефонов
org_no_phones = {
    "gid": "org_002",
    "name": "ООО Без телефонов", 
    "phones": []  # Пустой массив телефонов
}
# Результат: пропуск с причиной "no_org_phones"

# Низкий confidence
contact_low_confidence = {
    "gid": "contact_003",
    "name": "Сидоров",  # Только имя
    "organization_id": "org_001",
    "phones": []
    # Нет email, должности, города - низкий confidence
}
# Результат: отклонение с причиной "low_confidence"
```

### Метаданные обогащения

Система генерирует детальные метаданные для каждого контакта:
  ...
  "enrichment_metadata": [
    {
      ...
      "decision": {
        "decision": "auto_accept",
        "confidence": 0.8,
        "reason": "Высокий уровень уверенности (0.80 ≥ 0.75)"
      },
      "skip_reasons": []
    }
  ],
...
### Структура обогащенного телефона

Добавляемые телефоны содержат метаданные обогащения:
...
```json
{
  ...
  "source": "org_enrichment",        // Источник обогащения
  "org_gid": "org_001",            // GID организации-источника
  "confidence": 0.8,               // Уровень уверенности
  "enriched_at": "2025-01-10T15:30:00Z"  // Время обогащения
}
```
...
- **contacts_processed**: Общее количество обработанных контактов
- **contacts_enriched**: Количество успешно обогащенных контактов  
- **phones_added_total**: Общее количество добавленных телефонов
- **phones_skipped_total**: Общее количество пропущенных телефонов
- **avg_confidence**: Средний уровень уверенности по всем решениям

#### Детализация пропусков:

- **low_confidence**: Низкий confidence score
- **no_org_phones**: Организация без телефонов
- **already_has_phones**: Контакт уже имеет телефоны
- **mobile_only**: Только мобильные номера (исключены)
- **duplicate**: Дубликат существующего телефона
- **no_organization**: Контакт не связан с организацией

### Интеграция с PostProcessor

ContactPhoneEnricher интегрирован в основной пайплайн постобработки на этапе после обогащения локации:
...
5. Обогащение ИНН организаций
6. Обогащение email организаций
7. Обогащение локации организаций
8. Безопасное обогащение локации контактов
9. 📞 Обогащение телефонов контактов ← НОВЫЙ ЭТАП
...
- **Исключение мобильных номеров**: Телефоны типа "mobile" не добавляются к контактам
- **Фильтрация по типам**: Только корпоративные типы телефонов (office, main, fax)
- **Проверка источников**: Приоритет LLM-источника над обогащением

#### Защита от дубликатов:
- **Нормализованное сравнение**: По полю normalized (E.164)
- **Сохранение приоритета**: LLM-телефоны имеют приоритет
- **Логирование конфликтов**: Детальная информация о дубликатах
...
**Оптимизации:**
- Индексирование организаций по GID
- Кэширование нормализованных телефонов
- Batch обработка метаданных
...
**Запуск тестов:**
```bash
python -m pytest tests/test_contact_phone_enricher.py -v
```

**Покрытие тестами:**
- ✅ Система scoring (все факторы)
- ✅ Принятие решений (все пороги)
- ✅ Фильтрация телефонов (все типы)
- ✅ Проверка дубликатов (все сценарии)
- ✅ Edge cases (отсутствие данных, ошибки)
- ✅ Интеграция с PostProcessor
...
```python
import logging
logging.getLogger('contact_phone_enricher').setLevel(logging.DEBUG)

# Детальные логи confidence calculation
# Информация о принятых решениях
# Статистика фильтрации и дубликатов
```

#### Анализ метаданных:

```python
# Проверка решений по конкретному контакту
metadata = result['enrichment_metadata']
contact_meta = next(m for m in metadata if m['contact_gid'] == 'contact_001')
print(f"Decision: {contact_meta['decision']}")
print(f"Confidence: {contact_meta['confidence_scores']}")
```
...
# Получение статистики
stats = enricher.get_stats()
print(f"Success rate: {stats['contacts_enriched'] / stats['contacts_processed']:.2%}")
