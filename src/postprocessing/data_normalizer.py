...
class DataNormalizer:
    ...
    def __init__(self):
        ...
        self.logger.info(
            "📝 Используем простую нормализацию: только первое слово с заглавной буквы"
        )
    ...
    def _sanitize_phone_keys(self, phone: dict) -> dict:
        ...
        if removed:
            ...
            self.logger.debug(
                f"🧹 Sanitized phone keys: {removed} from {normalized_id}"
            )
    ...
    def normalize_contacts(
        ...
        for contact in contacts:
            try:
                ...
                self.logger.error(
                    f"Ошибка при нормализации контакта {contact.get('name', 'Unknown')}: {str(e)}"
                )
    ...
    def normalize_organizations(
        ...
        for org_id, org in organizations.items():
            try:
                ...
                self.logger.error(
                    f"Ошибка при нормализации организации {org.get('name', 'Unknown')}: {str(e)}"
                )
    ...
    def _normalize_single_organization(
        ...
        if normalized.get("name"):
            ...
            if normalized_name != original_name:
                self.logger.debug(
                    f"Нормализация организации: '{original_name}' → '{normalized_name}'"
                )
    ...
    def _normalize_organization_city(
        self, organization: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _enrich_organization_website(
        self, organization: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _normalize_organization_phones(
        self, organization: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if isinstance(entry, dict):
            ...
            if llm_phone_fields.issubset(entry.keys()):
                ...
                if entry.get("normalized") and entry["normalized"].strip():
                    ...
                    self.logger.debug(
                        f"Обнаружен готовый phone объект от LLM с нормализацией: {entry}"
                    )
                    ...
                    if result_entry.get("normalized"):
                        ui_formatted = self._format_ui_from_e164(
                            result_entry["normalized"]
                        )
                        if ui_formatted:
                            result_entry["number"] = ui_formatted
                            self.logger.debug(
                                f"📱 Regenerated UI format: {result_entry['normalized']} → {ui_formatted}"
                            )
                    ...
                    self.logger.debug(
                        f"Обнаружен LLM phone объект без нормализации: {entry}"
                    )
                    ...
                    metadata = {
                        ...
                        not in {
                            "normalized"
                        }  # Оставляем number, original, но пересчитываем normalized
                    }
                ...
                if "extension" in entry and entry["extension"]:
                    ext_pattern = (
                        rf"\s*\(\s*доб\.?\s*{re.escape(str(entry['extension']))}\s*\)"
                    )
                    raw_phone = re.sub(
                        ext_pattern, "", raw_phone, flags=re.IGNORECASE
                    ).strip()
            else:
            ...
            try:
                ...
                for phone_obj in normalized_phones:
                    ...
                    final_phone = {
                        **phone_obj,
                        **metadata,
                    }  # Приоритет metadata (включая type от LLM)
                    ...
                    if normalized_value and not normalized_value.startswith("+7"):
                        ...
                        if len(digits_only) == 10:  # 10 цифр без кода страны
                            ...
                            self.logger.debug(
                                f"Исправлена нормализация номера: {normalized_value} → {final_phone['normalized']}"
                            )
                    ...
                    if final_phone.get("normalized"):
                        ...
                        if ui_formatted:
                            ...
                            self.logger.debug(
                                f"📱 Regenerated UI format: {final_phone['normalized']} → {ui_formatted}"
                            )
                ...
                self.logger.error(
                    f"Ошибка нормализации телефона через PhoneNormalizer '{raw_phone}': {exc}"
                )
    ...
    def _normalize_phone_variants(
        self, raw_phone: str, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        ...
        for variant in variants:
            ...
            normalized_digits = "".join(
                filter(str.isdigit, variant.get("normalized") or "")
            )
            ...
            if record.get("normalized"):
                ...
                if ui_formatted:
                    ...
                    self.logger.debug(
                        f"📱 Regenerated UI format (fallback): {record['normalized']} → {ui_formatted}"
                    )
    ...
    def _normalize_contact_position(self, contact: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if contact.get("position"):
            ...
            if normalized_position and normalized_position != original_position:
                self.logger.debug(
                    f"Нормализация должности: '{original_position}' → '{normalized_position}'"
                )
    ...
    def _normalize_organization_name(self, name: str) -> str:
        ...
        for word in words:
            cleaned_word = word
            for open_quote, close_quote in quote_pairs:
                if cleaned_word.startswith(open_quote) and cleaned_word.endswith(
                    close_quote
                ):
