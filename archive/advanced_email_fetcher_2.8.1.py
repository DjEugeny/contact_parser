...
def setup_logging(logs_dir: Path, start_date: datetime, end_date: datetime):
    ...
    logger.info(f"📝 Лог сохраняется в: {log_path}")
    logger.info("="*70)
    ...
    def is_sender_blacklisted(self, from_addr: str) -> Optional[str]:
        ...
        for blacklisted in self.blacklist:
            if blacklisted.startswith('*@'):
                # Проверка домена с маской: *@example.com
                domain = blacklisted[2:]
                if f"@{domain}" in from_addr_lower:
                    return f"домен в черном списке ({domain})"
            elif blacklisted.endswith('@*'):
                # Проверка пользователя с маской: user@*
                user = blacklisted[:-2]
                if from_addr_lower.startswith(user):
                    return f"пользователь в черном списке ({user})"
            elif blacklisted.endswith('*'):
                # 🔧 ИСПРАВЛЕНО: Точная проверка префикса info@*
                import re
                prefix = re.escape(blacklisted[:-1])
                if re.match(f"^{prefix}@", from_addr_lower):
                    return f"адрес начинается с '{blacklisted[:-1]}@' (в черном списке)"
            else:
                # Точное совпадение
                if blacklisted == from_addr_lower:
...
class AdvancedEmailFetcherV2:
    ...
    def safe_fetch(self, msg_id: bytes, flags: str = '(RFC822)') -> Optional[List]:
        """🛡️ Безопасное получение письма с агрессивными таймаутами"""
        
        for attempt in range(MAX_RETRIES):
            try:
                self.logger.info(f"   🔄 Попытка fetch {attempt + 1}/{MAX_RETRIES}")
                self.logger.info(f"   📨 MSG_ID: {msg_id}, флаги: {flags}")
                ...
                try:
                    ...
                    self.logger.info(f"   ✅ NOOP: {status}")
                except Exception as e:
                    self.logger.warning(f"   ⚠️ NOOP failed: {e}")
                    raise ConnectionError("IMAP connection lost")
                
                # 🆕 АГРЕССИВНЫЙ ТАЙМАУТ - 15 секунд вместо 30
                import signal
                def timeout_handler(signum, frame):
                    raise TimeoutError("Fetch операция превысила 15 секунд")
                
                signal.signal(signal.SIGALRM, timeout_handler)
                signal.alarm(15)  # 15 секунд
                
                try:
                    # Засекаем время операции
                    fetch_start = time.time()
                    self.logger.info(f"   🚀 Отправляем FETCH запрос (таймаут 15 сек)...")
                    ...
                    self.logger.info(f"   📥 FETCH ответ за {fetch_time:.2f} сек: {status}")
                    
                    if status == 'OK':
                        if data and data:
                            # 🔧 ИСПРАВЛЕНО: логируем правильно структуру данных
                            self.logger.info(f"   ✅ Данные получены: тип={type(data)}, элементов={len(data)}")
                            return data
                        else:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    self.logger.error(f"   ❌ Письмо пропущено после таймаута")
                    return None
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    if not self.connect():
                        self.logger.error(f"   ❌ Не удалось переподключиться")
                        continue
                else:
                    self.logger.error(f"   ❌ Письмо пропущено после {MAX_RETRIES} попыток")
                    return None
                    
            except Exception as e:
                self.logger.error(f"   ❌ Неожиданная ошибка на попытке {attempt + 1}: {e}")
    ...
    def extract_raw_email(self, fetch_data) -> Optional[bytes]:
        ...
        try:
            ...
            if isinstance(fetch_data, (list, tuple)):
                ...
                try:
                    if len(fetch_data) > 0 and isinstance(fetch_data[0], bytes) and len(fetch_data[0]) > 500:
    ...
    def detect_large_attachments_from_structure(self, structure_str: str) -> bool:
        ...
        try:
            ...
            # Если нет attachment или application - значит нет вложений
            if 'attachment' not in structure_lower and 'application' not in structure_lower:
                self.logger.info(f"   ✅ В структуре письма нет указаний на вложения")
                return False
            
            # Ищем опасные типы файлов ТОЛЬКО если есть вложения
            large_indicators = [
            ...
            # Ищем большие размеры ТОЛЬКО в контексте вложений
            import re
            # Ищем размеры только рядом с application или attachment
            attachment_sections = re.findall(r'(application[^)]+\d+|attachment[^)]+\d+)', structure_lower)
            
            for section in attachment_sections:
                size_matches = re.findall(r'(\d+)', section)
                for size_str in size_matches:
                    size = self.safe_parse_size(size_str)
                    if size > 5_000_000:  # Больше 5MB
                        self.logger.info(f"   ⚠️ Обнаружен большой файл: {size} байт")
                        return True
    ...
    def process_single_email(self, msg_id: bytes, date_str: str, email_num_in_day: int, total_emails_in_day: int) -> Optional[Dict]:
        ...
        # 🔧 ИНИЦИАЛИЗАЦИЯ ВСЕХ ПЕРЕМЕННЫХ В НАЧАЛЕ (исправление UnboundLocalError)
        attachments = []
        attachments_stats = {
            'total': 0, 
            'saved': 0, 
        ...
        self.logger.info(f"🔍 ОБРАБОТКА ПИСЬМА {email_num_in_day}/{total_emails_in_day}")
        self.logger.info(f"   MSG_ID: {msg_id}")
    ...
    def fetch_emails_by_date_range(self, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        while current_date <= end_date:
            ...
            if len(msg_ids) > 0:
                ...
                saved_today = len([e for e in all_emails if e])
                self.logger.info(f"✅ Сохранено писем за день: {saved_today}")
            else:
