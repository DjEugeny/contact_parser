...
"""
📧 Продвинутый IMAP-парсер v2.11 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ И УЛУЧШЕНИЯ
...
class AdvancedEmailFetcherV2:
    """🔥 Продвинутый парсер v2.11 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    def process_single_email(self, msg_id: bytes, date_str: str, email_num_in_day: int, total_emails_in_day: int) -> Optional[Dict]:
        ...
        try:
            ...
            # ❗️ ИЗМЕНЕНИЕ: Используем Message-ID для надежной проверки дубликатов
    ...
    def print_final_stats(self):
        ...
        self.logger.info(f"📁 Уже существовало писем: {self.stats.get('already_exists', 0)}")  # ✅ ДОБАВИТЬ эту строку
    ...
    def check_email_already_saved(self, message_id: Optional[str], date_folder: str) -> bool:
        """📁 Надежная проверка, существует ли уже сохраненное письмо по Message-ID."""
        if not message_id:
            # Если у письма нет Message-ID, мы не можем надежно проверить дубликат.
            # Безопаснее считать, что его нет, чем рисковать пропуском.
            return False
        ...
        try:
            ...
            # Ищем все JSON файлы в папке за этот день
            for json_file in emails_date_dir.glob('*.json'):
                try:
                    with open(json_file, 'r', encoding='utf-8') as f:
                        email_data = json.load(f)
                        if email_data.get('message_id') == message_id:
                            self.logger.info(f"📁 Письмо уже сохранено (Message-ID: {message_id[:30]}...), пропускаем.")
                            return True
                except (json.JSONDecodeError, KeyError, OSError):
                    # Пропускаем битые/нечитаемые файлы или файлы без нужного ключа
                    continue

            return False

        except Exception as e:
            self.logger.warning(f"⚠️ Ошибка проверки существования письма по Message-ID: {e}")
            return False


def main():
    """🚀 Главная функция для тестирования парсера v2.11 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    logger.info("📧 ПРОДВИНУТЫЙ IMAP-ПАРСЕР v2.11 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ")
    logger.info("="*75)
