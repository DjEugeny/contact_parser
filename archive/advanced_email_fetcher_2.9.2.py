...
"""
📧 Продвинутый IMAP-парсер v2.9.1 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ И УЛУЧШЕНИЯ
...
class AdvancedEmailFetcherV2:
    """🔥 Продвинутый парсер v2.9.1 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    def check_email_size(self, msg_id: bytes) -> int:
        ...
        try:
            ...
            if status == 'OK' and data:
                size_str = str(data[0]).split()[-1].rstrip(')')
                return int(size_str)
        except Exception:
            return 0
    ...
    def process_single_email(self, msg_id: bytes, date_str: str, email_num_in_day: int, total_emails_in_day: int) -> Optional[Dict]:
        ...
        try:
            ...
            if email_size > 50_000_000:  # 50MB
                self.logger.warning(f"⚠️ Очень большое письмо ({email_size} байт), пропускаем")
                self.stats['skipped_large_emails'] += 1
                return None
            elif email_size > 10_000_000:  # 10MB
                self.logger.info(f"📏 Большое письмо ({email_size} байт)")
...
def main():
    """🚀 Главная функция для тестирования парсера v2.9.1 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    logger.info("📧 ПРОДВИНУТЫЙ IMAP-ПАРСЕР v2.9.1 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ")
    logger.info("="*75)
