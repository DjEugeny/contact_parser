...
"""
📧 Продвинутый IMAP-парсер v2.10 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ И УЛУЧШЕНИЯ
...
class AdvancedEmailFetcherV2:
    """🔥 Продвинутый парсер v2.10 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    def __init__(self, logger):
        ...
        self.enable_size_logging = False  # По умолчанию отключено для продакшена
    ...
    def remove_skipped_email(self, msg_id: bytes):
        """🗑️ Удаление письма из списка пропущенных (после успешной обработки)"""
        skipped_path = self.data_dir / 'skipped_emails.json'
    ...
    def retry_skipped_emails(self):
        ...
        for key, info in list(skipped.items()):
            attempts = info.get('attempts', 0)
            date_str = info.get('date', '')
            reason = info.get('reason', 'unknown')
            
            if attempts >= 3:
                self.logger.warning(f"⚠️ Превышено число попыток для письма {key}, окончательно пропускаем")
                failed_retries += 1
                continue
        ...
        self.logger.info(f"❌ Осталось в очереди: {failed_retries}")
        self.logger.info("=" * 70)
...
def main():
    """🚀 Главная функция для тестирования парсера v2.10 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    logger.info("📧 ПРОДВИНУТЫЙ IMAP-ПАРСЕР v2.10 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ")
    logger.info("="*75)
