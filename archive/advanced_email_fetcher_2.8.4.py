...
class AdvancedEmailFetcherV2:
    """🔥 Продвинутый парсер v2.8 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    def parse_recipients(self, recipients_str: str) -> List[str]:
        """📧 ИСПРАВЛЕННОЕ название метода (было parse_recipient)"""
    ...
    def process_single_email(self, msg_id: bytes, date_str: str, email_num_in_day: int, total_emails_in_day: int) -> Optional[Dict]:
        ...
        try:
            ...
            # Парсим получателей
            try:
                to_emails = self.parse_recipients(to_addr)
                cc_emails = self.parse_recipients(cc_addr)
            except Exception as e:
            ...
            # 🔧 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Добавляем отладочный лог для черного списка
            self.logger.info(f"DEBUG: Проверяем адрес отправителя: '{from_addr}'")
...
def main():
    """🚀 Главная функция для тестирования парсера v2.8 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ"""
    ...
    logger.info("📧 ПРОДВИНУТЫЙ IMAP-ПАРСЕР V2.8 - ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ БАГОВ")
    logger.info("="*75)
